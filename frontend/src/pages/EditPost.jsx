...
const EditPost = () => {
  ...
  const [content, setContent] = useState(post[0]?.desc)
  const [contentHtml, setContentHtml] = useState(post[0]?.desc)
  ...
  setErrors({
    ...
    setIsLoading(true)
      ...
      await postAPI.updatePost({data:updateData,id:post[0]._id,token})
      dispatch(updatePost({postData:updateData,id:post[0]._id}))  
    navigate(`/post/${post[0]._id}`)
  };
  ...
  return !postList ? <div>Loading...</div>: (
    <div className="w-full flex flex-col gap-4 pt-4">
      <div className="w-full ">
        ...
        >
          Update {isLoading ? <div className="h-5 w-5 flex">
            <Loader h={'full'} w={'full'} color={'text-white'}/>
            </div>:""}
        </button>
      </div>
